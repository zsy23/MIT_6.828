	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	int r;
	if ((r = envid2env(srcenvid, &srce, 1)) < 0 || (r = envid2env(dstenvid, &dste, 1)) < 0)
		return r;
	...
	if (!(*pte & PTE_W) && (perm & PTE_W))
		return -E_INVAL;
	r = page_insert(dste->env_pgdir, pg, dstva, perm);
	return r;
}
...
{
	user_mem_assert(curenv, data, E1000_RX_PKT_SIZE, PTE_U | PTE_P | PTE_W);
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	switch(syscallno) {
	case SYS_cputs:
		user_mem_assert(curenv, (void *)a1, (size_t)a2, 0);
