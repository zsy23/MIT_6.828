...
env_setup_vm(struct Env *e)
{
	...
	for (uint32_t i = 0; i < ~0 - UTOP + 1; i += PTSIZE)
...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	for (char *i = ROUNDDOWN((char *)va, PGSIZE); i < ROUNDUP((char *)va + len, PGSIZE); i += PGSIZE) {
		struct PageInfo *pg = page_alloc(0);
		if (pg)
			page_insert(e->env_pgdir, pg, (void *)i, PTE_U | PTE_W);
		else
